...
const closest = col => {
  if (isHexColor(col)) {
    ...
    for (let color of colors) {
      const distance = colorDistance(normalizeHex(col), color[0])
...
const colorDistance = (col1, col2) => {
  const c1 = col1.substr(1)
  const c2 = col2.substr(1)
  const distR = Math.abs(hex2dec(c1.substr(0, 2)) - hex2dec(c2.substr(0, 2)))
  const distG = Math.abs(hex2dec(c1.substr(2, 2)) - hex2dec(c2.substr(2, 2)))
  const distB = Math.abs(hex2dec(c1.substr(4, 2)) - hex2dec(c2.substr(4, 2)))
...
const normalizeHex = color => {
  const c = stripHashtag(color).toUpperCase()
  return (c.length > 3) ? '#' + c : '#' + c[0] + c[0] + c[1] + c[1] + c[2] + c[2]
}

const stripHashtag = color => color.replace(/#/g, '')
